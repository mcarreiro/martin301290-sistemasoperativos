...
vector<vector<RWSemaphoreLock> > tablero_letras_sem;
RWSemaphoreLock palabras_sem;
...
int main(int argc, const char* argv[]) {
	...
	palabras_sem = RWSemaphoreLock();
	tablero_letras_sem = vector<vector<RWSemaphoreLock> >(alto);
	for (unsigned int i = 0; i < alto; ++i) {
		tablero_letras_sem[i] = vector<RWSemaphoreLock>(ancho, RWSemaphoreLock());
	}
	...
	while (true) {
		if ((socketfd_cliente = accept(socket_servidor, (struct sockaddr*) &remoto, (socklen_t*) &socket_size)) == -1)
			...
			//close(socket_servidor); //No hay que cerrar la conexión
			pthread_t player; 
			pthread_create(&player, NULL , wrapper, (void*) &socketfd_cliente);
...
void* wrapper(void* socket){
	atendedor_de_jugador(*((int*)socket));
	return NULL;
}
	...
	while (true) {
		...
		if (comando == MSG_LETRA) {
			...
			if (es_ficha_valida_en_palabra(ficha, palabra_actual)) {
				...
				tablero_letras_sem[ficha.fila][ficha.columna].wlock(); //Como vamos a escribir necesitamos lock de escritura
				tablero_letras[ficha.fila][ficha.columna] = ficha.letra;
				tablero_letras_sem[ficha.fila][ficha.columna].wunlock(); //Devolvemos el lock de escritura				
				// OK
...
void quitar_letras(list<Casillero>& palabra_actual) {
	 //Como vamos a escribir b
	for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
		tablero_letras_sem[casillero->fila][casillero->columna].wlock();
		tablero_letras[casillero->fila][casillero->columna] = VACIO;
		tablero_letras_sem[casillero->fila][casillero->columna].wunlock();
	}
	palabra_actual.clear();
}
...
bool es_ficha_valida_en_palabra(const Casillero& ficha, const list<Casillero>& palabra_actual) {
	...
	tablero_letras_sem[ficha.fila][ficha.columna].rlock(); //Devolvemos el lock de escritura
	if (tablero_letras[ficha.fila][ficha.columna] != VACIO) {
		tablero_letras_sem[ficha.fila][ficha.columna].runlock(); //Devolvemos el lock de escritura
		return false;
	}
	tablero_letras_sem[ficha.fila][ficha.columna].runlock(); //Devolvemos el lock de escritura
